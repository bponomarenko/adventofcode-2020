...
#[derive(Clone)]
struct Game {
    boxes: HashMap<i8, HashMap<i8, HashMap<i8, u8>>>,
...
impl Game {
    fn active_count(&self) -> u32 {
        self.boxes.values().fold(0, |slice_acc: u32, slice| {
            slice
                .values()
    ...
    fn active_neighbours_count(&self, z: &i8, y: &i8, x: &i8) -> u32 {
        let mut count = 0;
        for dz in (z - 1)..=(z + 1) {
            for dy in (y - 1)..=(y + 1) {
                for dx in (x - 1)..=(x + 1) {
                    if dz != *z || dy != *y || dx != *x {
                        count += *self.get_value(&dz, &dy, &dx) as u32;
                    }
                }
            }
        }
    ...
    fn set_value(&mut self, z: &i8, y: &i8, x: &i8, value: u8) {
        self.boxes
            .entry(*z)
    ...
    fn get_value(&self, z: &i8, y: &i8, x: &i8) -> &u8 {
        let slice = self.boxes.get(z);
        if slice.is_none() {
            return &0;
        }
        ...
        for z in clone.z_range.clone() {
            for y in clone.y_range.clone() {
                for x in clone.x_range.clone() {
                    let current_value = *clone.get_value(&z, &y, &x);
                    let active_neighbours_count = clone.active_neighbours_count(&z, &y, &x);
                    ...
                    self.set_value(&z, &y, &x, new_value);
                }
            }
        }
    }
...
pub fn run(input: String) -> u32 {
    ...
    input.split_ascii_whitespace().enumerate().for_each(|(y, row)| {
        row.chars().enumerate().for_each(|(x, char)| {
            game.set_value(&0, &(y as i8), &(x as i8), if char == '#' { 1 } else { 0 });
        });
    });
